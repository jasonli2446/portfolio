...
export default {
  ...
  init: (win) => {
    ...
    function update() {
      ...
      // Simulated metrics with jitter
      const cpu = 15 + openWindows * 8 + totalClones * 3 + Math.random() * 10;
      const mem = openWindows * 2.4 + totalClones * 0.8 + 4 + Math.random() * 2;
      const net = totalClones * 12 + Math.random() * 30;
